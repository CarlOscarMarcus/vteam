  ...
  <script>
    ...
    function handleMessage(event){
      ...
      if(data.type==='position'){
        ...
        if(!window.userMarker){ window.userMarker=L.marker([latitude,longitude]).addTo(map); map.setView([latitude,longitude],15); }
      ...
      function updateMarkers(key, icon){
        ...
        window[key]=[];
        data.items.forEach(item=>{ const m=L.marker([item.lat,item.lng],{icon}).addTo(map); window[key].push(m); });
      }
...
export default function Map() {
  ...
  useEffect(() => {
    (async () => {
      ...
      Location.watchPositionAsync({accuracy: Location.Accuracy.High,distanceInterval:5}, loc=>{
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <View style={styles.mapContainer}>
        <WebView ref={webviewRef} source={{html: leafletHTML}} originWhitelist={["*"]} javaScriptEnabled domStorageEnabled style={{flex:1}} />
      </View>
    </ThemedView>
  );
}
